...
// We will use regex to replace everything from `{[ ` down to `}))}` just before `</div>` inside `<div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 ...`
// The original starts with `{[ ` and ends with `           </div>`
// Let's do a substring replace
const startIndex = content.indexOf("{[ \n                  { name: 'Facebook', icon: Facebook");
const endIndexStr = "                ))}             </div>          </div>                    <TestimonialSlider />";
// let's just find the `))}             </div>          </div>` part

const endRegex = /}\)\)}\\s*<\/div>/;

if (startIndex !== -1) {
    const stringToReplace = content.substring(startIndex, content.indexOf("</div>", startIndex));
    // wait indexing is safer using split or regex:
}

// better regex
const pattern = /\{\[\s*\{\s*name:\s*'Facebook'(.*?)\]\.map\(\s*\(social,\s*idx\)\s*=>\s*\([\s\S]*?\}\)\)\}/m;
