...
const afterRender = `{(adminTab === "topups" || adminTab === "withdrawals") && (
              <div className="flex gap-2 mb-4 bg-gray-100 p-1.5 rounded-xl w-full max-w-sm">
                <button
...
c = c.replace(pendingFilterOld, pendingFilterNew);
c = c.replace(pendingFilterOld, pendingFilterNew); // There are two occurrences
